...
select
       ...
       max(if(o.obs_group in(141814,160658) or o.concept_id=165116,o.value_coded, "" )) as prev_gbv_survivor,
       max(if(o.obs_group in(141814,160658) or o.concept_id =165117,o.value_coded, "" )) as physical_ipv,
       max(if(o.obs_group in(141814,160658) or o.concept_id =165034,o.value_coded, "" )) as emotional_ipv,
       max(if(o.obs_group in(141814,160658) or o.concept_id =165070,o.value_coded, "" )) as sexual_ipv,
       max(if(o.obs_group in(141814,160658) or o.concept_id =165045,o.value_coded, "" )) as current_gbv_survivor,
       e.date_created as date_created,
       if(max(o.date_created) > min(e.date_created),max(o.date_created),NULL) as date_last_modified,
       e.voided as voided
from encounter e
       inner join person p on p.person_id=e.patient_id and p.voided=0
       inner join kenyaemr_etl.etl_patient_demographics de on p.person_id=de.patient_id
       ...
       INNER JOIN (SELECT o.encounter_id as encounter_id,o.person_id, o.obs_id,o.concept_id as obs_group,o1.concept_id as concept_id, o1.value_coded as value_coded,o1.value_text as value_text,
       o1.date_created,o1.voided
		from obs o join obs o1 on o.obs_id = o1.obs_group_id and o1.concept_id in (160658,141814,165116,158358,165117,118688,165034,
        152370,165070,1582,165045,165229,60632))o on o.encounter_id = e.encounter_id
where e.voided=0 
group by e.encounter_id
--select
--       e.uuid,e.creator,e.patient_id,e.visit_id, date(e.encounter_datetime) as visit_date, e.location_id, e.encounter_id,
